...
{
    /// <summary>
    /// Data access layer for Central Coast v3 tables.
    /// Uses <see cref="CentralCoastV3DbContext"/> for all writes.
    /// </summary>
    public class CentralCoastV3DAL
    {
        private readonly string _connectionString;

        public CentralCoastV3DAL() : this(ConnectionHelper.GetConnectionString())
        {
        }

        public CentralCoastV3DAL(string connectionString)
        {
        ...
        public bool AddCubeDataRow(CubeDataRowV3 row)
        {
            try
            {
                using var db = new CentralCoastV3DbContext(_connectionString);
        ...
        /// </summary>
        public int AddCubeDataRows(IEnumerable<CubeDataRowV3> rows)
        {
            try
            {
                using var db = new CentralCoastV3DbContext(_connectionString);
        ...
        /// </summary>
        public (CentralCoastV3DbContext db, Dictionary<(int dateIdx, int zoneID, long patchID), CubeDataRowV3> lookup)
            OpenCubeDataLookup(string scenarioRunId, int scenarioIdx)
        {
            var db = new CentralCoastV3DbContext(_connectionString);
            var lookup = db.CubeData
                .Where(r => r.scenarioRunId == scenarioRunId && r.scenarioIdx == scenarioIdx && r.patchID > 0).Cast<CubeDataRowV3>()
        ...
        /// </summary>
        public static int SaveCubeDataRows(CentralCoastV3DbContext db, int chunkSize = 5_000)
        {
            var changed = db.ChangeTracker.Entries<CubeDataRowV3>()
        ...
        public static int SaveCubeDataRows(CentralCoastV3DbContext db, IEnumerable<CubeDataRowV3> rows, int chunkSize = 5_000)
        {
        ...
        // public bool AddBurnData(BurnDataRow row)
        // {
        //     try
        //     {
        ...
        /// </summary>
        public int AddDateRows(IEnumerable<Date> rows, int chunkSize = 5000)
        {
            int total = 0;
            try
            {
