...
{
    /// <summary>
    /// Central Coast v3 precomputed terrain frame (table <c>TerrainData</c>).
    ///
    /// One row per (scenarioRunId, scenarioIdx, year, month). The <c>_dataList</c>
    ...
    /// Big Creek TerrainData rows are in a separate table/context and are untouched. Central Coast rows are in a separate table/context.
    /// </summary>
    [Table("TerrainData")]
    [Index(nameof(scenarioRunId), nameof(scenarioIdx), nameof(year), nameof(month))]
    public class TerrainDataRowV3
    {
        [Key]
