		...
		public void GenerateHeights (Terrain terrain, float tileSize)
		{
			...
			for (int i = 0; i < terrain.terrainData.heightmapResolution; i++) {
				for (int k = 0; k < terrain.terrainData.heightmapResolution; k++) {
					heights [i, k] = Mathf.PerlinNoise (((float)i / (float)terrain.terrainData.heightmapResolution) * tileSize + randomMove.x, ((float)k / (float)terrain.terrainData.heightmapResolution) * tileSize + randomMove.y) / 10.0f;
				}
			}
