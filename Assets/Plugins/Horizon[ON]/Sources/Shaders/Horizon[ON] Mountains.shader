...
Shader "Horizon[ON]/Horizon[ON] Mountains" {
    Properties {
        ...
        [NoScaleOffset]_GlobalDiffuse ("Global Diffuse", 2D) = "gray" {}
    ...
    SubShader {
        ...
        Pass {
            ...
            uniform sampler2D _GlobalDiffuse;
            ...
            float4 frag(VertexOutput i) : COLOR {
                ...
                float3 _GrassNormal_var = UnpackNormal(tex2D(_GrassNormal,TRANSFORM_TEX(i.uv0, _GrassNormal)));
                ...
                float3 node_6497_nrm_base = _GlobalNormal_var.rgb + float3(0,0,1);
                float3 node_6497_nrm_detail = lerp(lerp(_RockNormal_var.rgb,_GrassNormal_var.rgb,SplatGrass),_DirtNormal_var.rgb,SplatDirt) * float3(-1,-1,1);
                float3 node_6497_nrm_combined = node_6497_nrm_base*dot(node_6497_nrm_base, node_6497_nrm_detail)/node_6497_nrm_base.z - node_6497_nrm_detail;
                float3 node_6497 = node_6497_nrm_combined;
                float3 NormalChain = normalize(node_6497);
...
////// Specular:
                ...
                float4 _GlobalDiffuse_var = tex2D(_GlobalDiffuse,i.uv0);
                ...
                float3 DiffuseChain = (_GlobalTint.rgb*saturate((lerp(lerp((_RockDiffuse_var.rgb*_RockTint.rgb),(_GrassDiffuse_var.rgb*_GrassTint.rgb),SplatGrass),(_DirtDiffuse_var.rgb*_DirtTint.rgb),SplatDirt)/(1.0-_GlobalDiffuse_var.rgb)))*AO);
                float node_4447 = 0.0;
