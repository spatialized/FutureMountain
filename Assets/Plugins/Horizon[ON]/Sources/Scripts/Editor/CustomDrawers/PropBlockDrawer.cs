...
namespace Horizon {
	public class PropBlockDrawer : MaterialPropertyDrawer {
		...
		public PropBlockDrawer(string prop1):base() {
			if (inactiveFlag1 = (prop1.IndexOf("INACTIVE")==0)) prop1=prop1.Substring(8);
			ProcessVisibilityProp1(prop1);
		}
		public PropBlockDrawer(string prop1, string prop2):base() {
			if (inactiveFlag1 = (prop1.IndexOf("INACTIVE")==0)) prop1=prop1.Substring(8);
		...
		bool checkEmissiveUsage(Material mat, string label) {
			if (label.ToLower().IndexOf("emissi")<0) return true;
			if (!mat.HasProperty("_Emissiveness")) return true;
			if (mat.GetFloat("_Emissiveness")==0) return false;
			return true;
		}
