Shader "NatureManufacture Shaders/Tree Leaves Metalic Snow"
{
	...
	SubShader
	{
		...
		void surf( Input i , inout SurfaceOutputStandardCustom o )
		{
			...
			float4 lerpResult51 = lerp( temp_output_97_0 , ( tex2D( _SnowAlbedoRGB, uv_SnowAlbedoRGB ) - fixed4( appendResult100 , 0.0 ) ) , clampResult93);
			...
			float2 panner205 = ( temp_cast_7 + clampResult271.x * float2( 10000,4000 ));
			float2 lerpResult209 = lerp( float2( 0,0 ) , panner205 , i.vertexColor.b);
			float2 temp_output_217_0 = ( lerpResult285 * ( lerpResult209 * float2( 0.0001,0.0001 ) ) );
			float mulTime274 = _Time.y * 0.9;
			float cos214 = cos( lerpResult262.x );
			float sin214 = sin( lerpResult262.x );
			...
			float2 clampResult215 = clamp( rotator214 , float2( 0.2,0.2 ) , float2( 0.8,0.8 ) );
			float2 lerpResult221 = lerp( temp_output_217_0 , ( temp_output_217_0 * float2( 0.45,0.45 ) ) , clampResult215.x);
			float3 appendResult222 = (fixed3(( ( i.vertexColor.r * _WindPower ) * ( ( appendResult198 * float2( 0.8,0.8 ) ) + ( appendResult272 + clampResult271 ) ) ).x , lerpResult221.x , ( ( i.vertexColor.r * _WindPower ) * ( ( appendResult198 * float2( 0.8,0.8 ) ) + ( appendResult272 + clampResult271 ) ) ).y));
			#ifdef _WINDVERTEXCOLORMAINRNOISEB_ON
				float3 staticSwitch224 = appendResult222;
			#else
				float3 staticSwitch224 = fixed3(0,0,0);
			#endif
			float4 transform225 = mul(unity_WorldToObject,fixed4( staticSwitch224 , 0.0 ));
			float clampResult245 = clamp( transform225.x , 0.0 , 1.0 );
			float4 lerpResult228 = lerp( temp_output_97_0 , ( temp_output_97_0 * fixed4( _WindColorMultiply , 0.0 ) ) , clampResult245);
			float4 lerpResult54 = lerp( ( _TranslucencyColor * ( lerpResult228 - fixed4( ( appendResult100 * _Snow_Amount ) , 0.0 ) ) ) , float4( 0,0,0,0 ) , temp_output_45_0);
