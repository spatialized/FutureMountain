...
namespace DynamicFogAndMist {
    ...
    public class DynamicFogBase : MonoBehaviour {
        ...
        public static class ShaderParams {
            public static int Alpha = Shader.PropertyToID("_FogAlpha");
            public static int Dither = Shader.PropertyToID("_FogDither");
            public static int ClipDir = Shader.PropertyToID("_ClipDir");
            public static int ClipToWorld = Shader.PropertyToID("_ClipToWorld");
             
            public static int Speed = Shader.PropertyToID("_FogSpeed");
            public static int NoiseData = Shader.PropertyToID("_FogNoiseData");
            public static int HeightData = Shader.PropertyToID("_FogHeightData");
            public static int Distance = Shader.PropertyToID("_FogDistance");

            public static int FoWTexture = Shader.PropertyToID("_FogOfWar");
            public static int FoWCenter = Shader.PropertyToID("_FogOfWarCenter");
            public static int FoWSize = Shader.PropertyToID("_FogOfWarSize");
            public static int FoWCenterAdjusted = Shader.PropertyToID("_FogOfWarCenterAdjusted");
            public static int FoWData = Shader.PropertyToID("_FogOfWarData");
            
            public static int SkyData = Shader.PropertyToID("_FogSkyData");
            public static int Color = Shader.PropertyToID("_FogColor");
            public static int Color2 = Shader.PropertyToID("_FogColor2");
            ...
            if (transitionAlpha) {
                if (targetFogAlpha >= 0) {
                    if (targetFogAlpha != currentFogAlpha || targetSkyHazeAlpha != currentSkyHazeAlpha) {
                        ...
                        fogMat.SetFloat(ShaderParams.Alpha, currentFogAlpha);
                        SetSkyData();
                    }
            ...
            if (transitionColor) {
                if (targetFogColors) {
                    if (targetFogColor1 != currentFogColor1 || targetFogColor2 != currentFogColor2) {
                        ...
                        fogMat.SetColor(ShaderParams.Color, currentFogColor1);
                        fogMat.SetColor(ShaderParams.Color2, currentFogColor2);
                    }
        ...
        protected void UpdateMaterialPropertiesNow() {
            ...
            fogMat.SetVector(ShaderParams.Speed, -_windDirection.normalized * sp);

            Vector4 noiseData = new Vector4(_noiseStrength, _turbulence, currentCamera.farClipPlane * 15.0f / 1000f, _noiseScale);
            fogMat.SetVector(ShaderParams.NoiseData, noiseData);
            ...
            fogMat.SetVector(ShaderParams.HeightData, heightData);
            ...
            fogMat.SetVector(ShaderParams.Distance, distanceData);
            ...
            if (fogOfWarEnabled) {
                ...
                fogMat.SetTexture(ShaderParams.FoWTexture, fogOfWarTexture);
                fogMat.SetVector(ShaderParams.FoWCenter, _fogOfWarCenter);
                fogMat.SetVector(ShaderParams.FoWSize, _fogOfWarSize);
                Vector3 ca = fogOfWarCenter - 0.5f * _fogOfWarSize;
                fogMat.SetVector(ShaderParams.FoWCenterAdjusted, new Vector3(ca.x / _fogOfWarSize.x, 1f, ca.z / _fogOfWarSize.z));
                shaderKeywords.Add("FOG_OF_WAR_ON");
            }
            if (_enableDithering) {
                fogMat.SetFloat(ShaderParams.Dither, _ditherStrength * 0.1f);
        ...
        void SetSkyData() {
            ...
            fogMat.SetVector(ShaderParams.SkyData, skyData);
        }
        ...
        void UpdateFogColor() {
            ...
            fogMat.SetColor(ShaderParams.Color, fogIntensity * currentFogColor1 * sunColor);
            fogMat.SetColor(ShaderParams.Color2, fogIntensity * currentFogColor2 * sunColor);
            Color sColor = fogIntensity * scatteringColor;
            fogMat.SetColor(ShaderParams.SunColor, new Vector4(sColor.r, sColor.g, sColor.b, scattering));
            fogMat.SetVector(ShaderParams.SunDir, -sunDirection);
        ...
        public void SetFogOfWarAlpha(Vector3 worldPosition, float radius, float fogNewAlpha, float smoothness) {
            if (fogOfWarTexture == null || fogOfWarColorBuffer == null || fogOfWarColorBuffer.Length == 0)
                return;
        ...
        /// <param name="fogNewAlpha">target alpha value.</param>
        /// <param name="smoothness">border smoothness.</param>
        public void SetFogOfWarAlpha(Bounds bounds, float fogNewAlpha, float smoothness) {
            if (fogOfWarTexture == null || fogOfWarColorBuffer == null || fogOfWarColorBuffer.Length == 0)
                return;
        ...
        void UploadFogOfWarTextureToGPU() {
